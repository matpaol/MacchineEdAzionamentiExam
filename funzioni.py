"""
funzioni.py — funzioni condivise dai notebook del progetto.
...
Indice:
    ...
    2. Lettura dei file .mat
    3. Inventario e controlli sui dati
    4. Statistiche dei segnali
    5. Segmentazione
    6. Frequenze di guasto e analisi spettrale
    7. Modelli: DAE e CNN
    8. Suddivisione, metriche e feature della Tabella 5
    9. Utilità per figure e tabelle
"""
...
def misura_fs(mat_file, nome_canale=config.CANALE_CORRENTE, struct=None,
              atteso=config.FS_ATTESO, durata_attesa=config.DURATA_ATTESA,
              tolleranza=config.TOLLERANZA):
    """
    Frequenza e durata effettive del canale indicato. Solleva ValueError se si
    discostano da quelle attese oltre la tolleranza: meglio fermarsi che
    proseguire con una lunghezza di frame sbagliata.
    """
    struct = struct if struct is not None else apri(mat_file)
    ...
    campioni = asse.size
    fs = (campioni - 1) / (asse[-1] - asse[0])
    durata = campioni / fs

    if abs(fs - atteso) / atteso > tolleranza:
        raise ValueError(f'frequenza misurata {fs:.3f} Hz incompatibile con {atteso} Hz')
    if abs(durata - durata_attesa) / durata_attesa > tolleranza:
        raise ValueError(f'durata misurata {durata:.6f} s incompatibile con {durata_attesa} s')
    return {'raster': raster, 'campioni': campioni,
            'frequenza_Hz': fs, 'durata_s': durata}
...
# ===========================================================================
# 4. Statistiche dei segnali (Parte 20 del corso, "Features Extraction")
# ===========================================================================
...
# ===========================================================================
# 5. Segmentazione (equazioni 7-9 del paper)
# ===========================================================================
...
def lunghezza_frame(rpm, fs=config.FS_ATTESO):
    ...
    A 1500 rpm e 64 kHz restituisce 2560.
    """
...
def costruisci_segmenti(inv, canale=config.CANALE_CORRENTE,
    ...
    Restituisce (X, anagrafica) dove ogni riga di X è un segmento e la
...
def ampiezza_a(frequenze, ampiezze, bersaglio, larghezza=2.0):
...
# ===========================================================================
# 7. Modelli: DAE e CNN (Tabelle 3 e 4 del paper)
# ===========================================================================
